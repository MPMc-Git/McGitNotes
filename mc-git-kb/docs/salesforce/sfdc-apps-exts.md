## Chrome Extensions for SFDC
#### Tested
- ORGanizer for Salesforce
- Salesforce Inspector Reloaded (replaced Salesforce Inspector)

#### Untested
- Advanced Code Searcher
- Advanced Lookup Extension
- Data Exporter for Salesforce
- Debug Logs Analyzer
- Enhance Salesforce Dashboard
- Lightning Inspector
- Org Check
- Salesforce Change Set Helper
- Salesforce DevTools
- Salesforce Enhanced Formula Editor
- Salesforce Hotkeys
- Salesforce Mass Editor
- Salesforce Simplified
- Salesforce Time Tracker
- SFDX Hardis
- SOQL Extractor & Analyzer
- User Role Helper
- Weflow
